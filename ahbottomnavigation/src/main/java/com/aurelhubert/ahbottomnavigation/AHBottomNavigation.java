...
public class AHBottomNavigation extends FrameLayout {
    ...
    private void updateNotifications(boolean updateStyle, int itemPosition) {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            ImageView ivDot = (ImageView) views.get(i).findViewById(R.id.iv_red_dot);
            ...
            if (updateStyle) {
                if (notificationItem.isShowDot()) {
                    notification.setVisibility(GONE);
                    if (notificationItem.getDotDrawable() > 0) {
                        ivDot.setImageResource(notificationItem.getDotDrawable());
                    }
